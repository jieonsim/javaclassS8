...
### ✔ 예매
#### ☑️ 날짜/회차 선택
1. 메인화면 / 스포츠 전체일정 / 구단별 예매하기 페이지에서 '예매하기' 버튼 클릭 시 새 창으로 예매 창이 띄워짐
2. 예매 창 접속 시 보안문자(CAPTCHA)를 생성하고 검증하는 기능 구현
3. 생성된 보안문자와 사용자가 입력한 보안문자를 비교해 일치하지 않을 경우 다음단계로 진입 불가
...
#### ☑️ 권종/할인/매수 선택
6. 선택한 매수만큼 원하는 권종 매수 선택
7. 선택한 권종 매수만큼 티켓 금액과 장당 1,000원의 예매수수료가 누적
8. 스포츠 예매권 권종 선택 시 마이페이지에서 미리 등록한 예매권을 사용하거나 권종 선택 단계에서 예매권 등록 후 사용해야함
9. 스포츠 예매권 권종 매수만큼은 티켓 금액과 예매수수료가 누적되지 않도록 처리

#### ☑️ 예매확인
10. 예매자 확인 및 취소기한, 취소수수료 동의 체크 후 결제하기 가능
11. 약관 미동의 시 결제하기 불가

#### ☑️ 결제
12. 포트원 결제 API를 이용하여 결제 후 예매 처리
13. 모든 매수를 스포츠 예매권으로 예매하여 결제 예정 금액이 0원일 경우 결제 없이 예매만 되도록 처리
