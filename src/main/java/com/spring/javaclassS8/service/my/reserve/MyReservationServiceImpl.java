...
public class MyReservationServiceImpl implements MyReservationService {
	...
	public Map<String, Object> getReservationDetail(int reservationId) {
	    ...
	    int sportId = reservation.getSportId();
	    // 취소 마감 시간 가져오기
	    int cancellationDeadlineMinutes = reservationDAO.getCancellationDeadlineMinutes(sportId);
	    ...
	    try {
	        ...
	        cancelDeadline.add(Calendar.MINUTE, -cancellationDeadlineMinutes);
	        Date cancelDeadlineDate = cancelDeadline.getTime();

	        reservation.setCancelDeadline(outputFormatDateTime.format(cancelDeadlineDate));
	        reservation.setFormattedCancelDeadline(new SimpleDateFormat("yyyy.MM.dd HH:mm").format(cancelDeadlineDate));

	        // 현재 시간과 취소 마감시간 비교
	        boolean isCancelable = new Date().before(cancelDeadlineDate);
	        reservation.setCancelable(isCancelable);
	        ...
	        if (isCancelable) {
	            ...
	            String cancelPeriod = outputFormatDateOnly.format(nextDay.getTime()) + "~" + outputFormatDateOnly.format(cancelDeadlineDate);
